...
public class MoviesController {
    ...
    public String getInsertForm(Model model) {
        if (!model.containsAttribute("movieCreateFormDto")) {
            model.addAttribute("movieCreateFormDto", new MovieCreateFormDto());
        }
    ...
    public String insertMovie(@Validated @ModelAttribute("movieCreateFormDto") MovieCreateFormDto movieCreateFormDto, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        if (bindingResult.hasErrors()) {
            logger.error("Validation error: {}", bindingResult.getAllErrors());
            redirectAttributes.addFlashAttribute("org.springframework.validation.BindingResult.movieCreateFormDto", bindingResult);
            redirectAttributes.addFlashAttribute("movieCreateFormDto", movieCreateFormDto);
            return "redirect:/admin/movies/new";
        }

        try {
            moviesService.save(movieCreateFormDto);
